...
def estimate_norm(lmk, image_size=112, mode='arcface'):
    ...
    sim_trans = trans.SimilarityTransform()
    sim_trans.estimate(lmk, dst)
    align_matrix = sim_trans.params[:2, :]
    return align_matrix

def norm_crop(img, landmark, image_size=112, mode='arcface'):
    align_matrix = estimate_norm(landmark, image_size, mode)
    warped = cv2.warpAffine(img, align_matrix, (image_size, image_size), borderValue=0.0)
    return warped

def run_one_image(face_rec, face_image):
...
def get_arcface(name="r50", model_path="../checkpoints/backbone.pth"):
...
def main():
    ...
    root_path = "./regular_users"
    
    app1 = FaceAnalysis(name='antelopev2', root='./models/insightface', providers=['CUDAExecutionProvider'])
    app2 = FaceAnalysis(name='buffalo_l', root='./models/insightface', providers=['CUDAExecutionProvider'])
    model = get_arcface().to(device='cuda')
    ...
    for r in range(20):
        ...
        for u, user in enumerate(users):
            if not cross_id:
                first = []
            print(user)
            image_paths = glob.glob(os.path.join(user, '*.*'))
            random.shuffle(image_paths)
            for i, image_path in enumerate(image_paths):
                image_pil = Image.open(image_path).convert("RGB")
                face = get_face_info(app1, image_pil)
                if face is None: continue
                id_emb = torch.tensor(face['embedding'])[None]

                face_image = image_pil.crop(face['bbox'])
                face2 = get_face_info(app1, face_image)
                if face2 is None: continue
                id_emb2 = torch.tensor(face2['embedding'])[None]

                face = get_face_info(app2, image_pil)
                if face is None: continue
                id_emb_b = torch.tensor(face['embedding'])[None]

                face_image = image_pil.crop(face['bbox'])
                face2 = get_face_info(app2, face_image)
                if face2 is None: continue
                id_emb_b2 = torch.tensor(face2['embedding'])[None]
                
                face_image_crop = norm_crop(np.array(image_pil), face['kps'])
                # Image.fromarray(face_image_).save(f'{i}.png')
                id_emb3 = run_one_image(face_rec=model, face_image=face_image_crop)
                if len(first) == 0:
                    first = [id_emb, id_emb2, id_emb_b, id_emb_b2, id_emb3]
                else:
                    res = []
                    for j, (emb1, emb2) in enumerate(zip(first, [id_emb, id_emb2, id_emb_b, id_emb_b2, id_emb3])):
                        res.append(torch.nn.functional.cosine_similarity(emb1.cpu(), emb2.cpu()).numpy()[0])
                    if not (cross_id and u == 0):
                        print(i, res)
                        user_res.append(res)
    print(np.mean(user_res, axis=0)) # [0.6468161  0.63058263 0.61496335 0.60016227 0.623125  ]
